...
int main(){
	...
	srand(time(NULL)); // gera uma seed baseada na hora atual para gerar numeros aleatorios pela funcao rand()
  ...
  al_install_keyboard();
	...
	musica = al_load_sample("../res/sound/musica.ogg");
	erou = al_load_sample("../res/sound/erou.ogg");
	comeu = al_load_sample("../res/sound/comeu.ogg");
	atirou = al_load_sample("../res/sound/atirou.ogg");
	morreu = al_load_sample("../res/sound/morreu.ogg");

	al_set_new_display_flags(ALLEGRO_RESIZABLE);
	tela = al_create_display(largura, altura); //cria a janela

	bg_game = al_load_bitmap("../res/img/bg_jogo.png");
	bg_inicio = al_load_bitmap("../res/img/bg_inicio.png");
	c_vida_1 = al_load_bitmap("../res/img/c_vida_1.png");
	c_vida_2 = al_load_bitmap("../res/img/c_vida_1.png");
	c_vida_3 = al_load_bitmap("../res/img/c_vida_1.png");
	bg_fim = al_load_bitmap("../res/img/bg_fim.png");

	icone = al_load_bitmap("../res/img/c_sushi0.png");

	al_set_display_icon(tela, icone);

	timer = al_create_timer(1.0 / fps);

	arial24 = al_load_font("../res/font/arial.ttf", 24, 0);
	...
	Botao botaoJogar(largura/2, 340, "../res/img/botao_jogar.png");
	Botao botaoSair(largura/2, 440, "../res/img/botao_sair.png");

	Botao botaoMenu(largura/2, 365, "../res/img/botao_menu.png");
	Botao botaoDesistir(largura/2, 450, "../res/img/botao_desistir.png");
	...
	// loop do jogo
	while(!acabou){
		ALLEGRO_EVENT evento; // cria variavel de evento
		al_wait_for_event(filaEventos, &evento); // espera por algum evento

		if(estado == menu){
			if(evento.type == ALLEGRO_EVENT_TIMER){
        al_set_window_title(tela, "Caracois Alados Famintos - Menu");
			...
			if(evento.type == ALLEGRO_EVENT_TIMER){
        al_set_window_title(tela, "Caracois Alados Famintos - Jogo");
				contador--;
          ...
          al_draw_bitmap(c_vida_1, 95, 487,0);
			...
			if(evento.type == ALLEGRO_EVENT_TIMER){
        al_set_window_title(tela, "Caracois Alados Famintos - Fim de Jogo");
